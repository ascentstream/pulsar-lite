...
> 分析日期: 2026-03-09（2026-06 更新：persistent Exclusive read_position + hole-aware ack 已落地）
...
- [x] 批量消息限制
- [x] 消费者移除
- [x] Persistent read_position + hole-aware 读（`read_from` / `ack_message_shared`）
- [x] Cumulative / individual ack 编排（`Subscription::acknowledge_message`）

### 缺失功能 (高优先级)
- [ ] **连接活性检查**（consumer 级僵尸检测）
- [ ] **竞态条件处理 (重试机制)**
- [ ] **Key Hash Range 过滤**
...
- [ ] 详细错误信息
- [ ] Unsubscribe 权限检查
- [ ] **显式 Redeliver 命令**（SingleActive 当前忽略，依赖 consumer close）
...
- ✅ Persistent Exclusive 基本可用（`tests/persist/`）
- ✅ 适合开发测试与 embedded 场景
- ⚠️ 生产环境仍需加强连接活性与竞态处理
- ⚠️ 不支持 Hash Range 过滤 (高级功能)

### 9.3 建议实施顺序

1. **第一阶段**: 连接活性检查 + 竞态处理 (保证基本可用性)
2. **第二阶段**: 详细错误信息 (改善调试体验)
3. **第三阶段**: Key Hash Range 过滤 (高级功能，按需实现)

> Persistent cursor / read_position 已在 2026-06 完成，不再作为后续 Exclusive 专项。
