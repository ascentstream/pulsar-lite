...
#### 1. partition 实例向逻辑 topic 归一化

现在对：

- `persistent://public/default/my-topic-partition-0`

这类分区实例访问，metadata 层不会为每个分区单独落一份 topic metadata，而是统一收口到：

- `persistent://public/default/my-topic`

这和原生 Pulsar 的 partitioned topic metadata 语义是一致的。
...
- 对 partitioned topic 的 subscription metadata 也归一化到逻辑 topic
...
- topic 名
- subscription 名
- partitioned topic 的 `partitioned / partition_count`
- 本地 JSON 快照
...
## 7. 当前 `MetadataSnapshot` 方案与原生 Pulsar 的结构差异
...
- persistent topic existence 独立存储
...
因此从数据结构角度看，当前方案会存在一定冗余：

- topic 里已经包含父层级信息
- snapshot 里又额外保留了一份 tenant / namespace 列表
...
但和原生 Pulsar 相比，这种方式的确更扁平，也更冗余。

### 7.4 这部分后续更接近原生 Pulsar 的改进方向

后续如果要继续向原生 Pulsar 靠拢，这部分可以按下面的方向演进：

1. 将当前单文件 `MetadataSnapshot` 拆分成按资源层级管理的 metadata store 抽象
2. tenant / namespace / partitioned topic metadata 改成分资源单独读写，而不是整份快照统一重写
3. topic existence 与 partitioned topic metadata 分开管理
