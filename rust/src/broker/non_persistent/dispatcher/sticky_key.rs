use crate::broker::dispatcher::sticky_key::{
    resolve_sticky_key as resolve_sticky_key_from_metadata, sticky_key_hash as compute_sticky_hash,
};
...
impl NonPersistentStickyKeyDispatcher {
    ...
    fn resolve_sticky_key(entry: &NonPersistentEntry) -> Vec<u8> {
        resolve_sticky_key_from_metadata(entry.metadata())
    }

    fn sticky_key_hash(sticky_key: &[u8]) -> i32 {
        compute_sticky_hash(sticky_key)
    }
