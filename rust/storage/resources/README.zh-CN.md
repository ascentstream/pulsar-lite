...
它负责把 Pulsar 的领域操作，例如确保 tenant、namespace、topic、subscription 存在，转换成对 metadata store 的状态修改。
...
`resources` crate 位于 metadata 之上。它不定义新的存储格式，也不直接拥有持久化逻辑。它的职责是把 Pulsar 的资源语义应用到任意实现了 `MetadataStore` 的后端上。
...
每个 public resource 操作都会先修改内存中的 metadata 状态。如果状态确实发生变化，最后最多只调用一次持久化。
