...
impl TenantResources {
    ...
    pub fn ensure_tenant<S: MetadataStore>(
        &self,
        metadata: &mut S,
        tenant: &str,
        version: u32,
    ) -> Result<()> {
        let changed = metadata.state_mut().insert_tenant_metadata(tenant);
    ...
    pub fn has_tenant<S: MetadataStore>(&self, metadata: &S, tenant: &str) -> bool {
