...
def parse_reader_output(text: str) -> dict[str, Any]:
    throughput = re.search(
        r"Aggregated throughput stats ---\s+(\d+) records received ---\s+([\d.]+) msg/s ---\s+([\d.]+) Mbit/s",
        text,
    )
    latency = re.search(
        r"Aggregated latency stats --- Latency: mean:\s+([\d.]+) ms - med:\s+([\d.]+) - 95pct:\s+([\d.]+) - 99pct:\s+([\d.]+) - 99\.9pct:\s+([\d.]+) - 99\.99pct:\s+([\d.]+) - 99\.999pct:\s+([\d.]+) - Max:\s+([\d.]+)",
    ...
    if not throughput or not latency:
        interval_matches = re.findall(
            r"Read throughput:\s+(\d+)\s+msg\s+---\s+([\d.]+)\s+msg/s\s+--+\s+([\d.]+)\s+Mbit/s.*?Latency: mean:\s+([\d.]+)\s+ms.*?99pct:\s+([\d.]+).*?Max:\s+([\d.]+)",
            text,
            re.S,
        )
        if not interval_matches:
