    ...
    for broker_name, broker_cfg in BROKERS.items():
        ...
        try:
            for scenario in [s for s in SCENARIOS if s.broker == broker_name]:
                ...
                print(
                    f"==> running {scenario.name} [{scenario.description}] on {service_url}",
                    flush=True,
                )
                ...
                try:
                    if scenario.kind == "produce":
                        ...
                        producer_cmd = perf_cmd(
                            "produce",
                            service_url,
                            scenario.feed_producer_args or [],
                            topic,
